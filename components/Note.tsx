...
const Note: React.FC<NoteProps> = ({ data, zPos, currentTime, theme = 'dark' }) => {
  ...
  return (
    <group position={position}>
        ...
        <group position={[0, 0, -NOTE_SIZE * 0.2]}>
            <Extrude args={[SPARK_SHAPE, EXTRUDE_SETTINGS]} castShadow receiveShadow>
                <meshPhysicalMaterial 
                    ...
                    metalness={isLight ? 0.0 : 0.1}
                    transmission={isLight ? 0.0 : 0.1} // No transmission in light mode for solid color
                    thickness={0.5}
                    emissive={color}
                    emissiveIntensity={isLight ? 0.3 : 0.8} // Lower emissive in light mode to prevent washed-out colors
                />
            </Extrude>
        </group>
      </group>
      ...
      <mesh position={[0, 0, NOTE_SIZE * 0.1]}>
         ...
         <meshBasicMaterial color={isLight ? "#ffffff" : "white"} toneMapped={false} transparent opacity={0.9} />
      </mesh>

      {/* Outer Wireframe Glow for emphasis */}
      <group position={[0, 0, -NOTE_SIZE * 0.2]}>
          <mesh>
             <extrudeGeometry args={[SPARK_SHAPE, { ...EXTRUDE_SETTINGS, depth: EXTRUDE_SETTINGS.depth * 1.15 }]} />
             <meshBasicMaterial color={isLight ? "#111111" : color} wireframe transparent opacity={isLight ? 0.4 : 0.3} />
          </mesh>
      </group>
    </group>
  );
